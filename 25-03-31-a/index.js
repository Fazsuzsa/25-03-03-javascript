...
app.post("/tiere", (req, res) => {
  try {
    ...
    const nameTaken = tiere.find(
      (tier) => tier.name == name && tier.art == art
    );
    if (nameTaken) {
      return res
        .status(400)
        .json({
          error: "Es gibt bereits ein Tier mit diesem Namen und dieser Art",
        });
    }
    ...
    if (name.length < 3) {
      return res
        .status(400)
        .json({ error: "Name muss mind. 3 Buchstaben beinhalten!" });
    }
